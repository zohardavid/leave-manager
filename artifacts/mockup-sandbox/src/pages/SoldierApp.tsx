...
function dayType(d: Date, round: string): "home" | "base" {
  if (d < DEPLOYMENT_START || d > DEPLOYMENT_END) return "base";
  if (d < ROUTINE_START) return "base";
  const cycleDay =
    Math.round((d.getTime() - ROUTINE_START.getTime()) / 86400000) %
    CYCLE_LENGTH;
  if (cycleDay === 0) return "base";
  if (cycleDay >= 1 && cycleDay <= 6)
    return round === "א" ? "home" : "base";
  return round === "א" ? "base" : "home";
}
...
function calendarWeeks(year: number, month: number) {
  ...
  const startOffset = (firstDay.getDay() + 0) % 7;
...
type Tab = "home" | "requests" | "round" | "calendar";
...
export default function SoldierApp({
  ...
  const load = useCallback(async () => {
    try {
      const [myReqs, allReqs, rnds, swps, sols] = await Promise.all([
  ...
  const myRound = rounds.find(
    (r) => r.soldier_name === soldier.name && r.status === "Approved",
  );
  const pendingRound = rounds.find(
    (r) => r.soldier_name === soldier.name && r.status === "Pending",
  );
  ...
  const daysLeft =
    today < DEPLOYMENT_START
      ? Math.round(
          (DEPLOYMENT_START.getTime() - today.getTime()) / 86400000,
        )
      : today <= DEPLOYMENT_END
        ? Math.round(
            (DEPLOYMENT_END.getTime() - today.getTime()) / 86400000,
          )
        : 0;
  ...
  const navItems: { key: Tab; icon: string; label: string }[] = [
    ...
    { key: "round", icon: "🔄", label: "סבב" },
    { key: "calendar", icon: "📅", label: "לוח" },
  ];

  return (
    <div className="flex flex-col h-full">
      <header className="bg-slate-800 text-white px-4 py-3 flex items-center justify-between shrink-0 safe-area-top">
        <div>
      ...
      <nav className="fixed bottom-0 inset-x-0 bg-white border-t border-gray-200 flex">
        {navItems.map((item) => (
          ...
          >
            ...
            <span className={tab === item.key ? "font-semibold" : ""}>
              {item.label}
            </span>
          </button>
        ))}
    ...
    <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <div className="text-2xl font-bold text-slate-800">
            {myRound
              ? `סבב ${myRound.round}'`
              : pendingRound
                ? `סבב ${pendingRound.round}'`
                : "—"}
          </div>
          <div className="text-sm font-medium text-gray-700 mt-0.5">סבב</div>
          {myRound && (
            <div className="text-xs text-green-600 mt-0.5">מאושר ✅</div>
          )}
          {pendingRound && !myRound && (
            <div className="text-xs text-yellow-600 mt-0.5">ממתין לאישור</div>
          )}
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
        ...
        <p className="text-xs text-amber-600 mt-1">
          מחזור 15 ימים החל מ-30.04.2026
        </p>
      </div>
    </div>
  );
  ...
  const submitSwap = async (e: React.FormEvent) => {
    ...
    try {
      await api.createSwap({
        requester: soldier.name,
        partner: swapPartner,
        start_date: swapStart,
        end_date: swapEnd,
      });
    ...
    <div className="p-4 space-y-4">
        ...
        <form onSubmit={submitRequest} className="space-y-4 bg-white rounded-2xl p-4 shadow-sm">
          ...
          {(() => {
            ...
            const approved = allRequests.filter(
              (r) => r.status === "Approved" && r.soldier_name !== soldier.name,
            );
            let warn = false;
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-slate-800 text-white py-3 rounded-xl font-semibold disabled:opacity-50 active:scale-95 transition-transform"
          >
          ...
          {requests.length === 0 ? (
            ...
            [...requests]
              ...
              .map((r) => (
                <div
                  key={r.id}
                  className={`rounded-2xl border p-4 ${STATUS_COLOR[r.status] ?? ""}`}
                >
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {diffDays(r.start_date, r.end_date)} ימים · {r.reason}
                  </div>
                  {r.commander_note && (
                    <div className="text-xs text-blue-700 mt-1.5 bg-blue-50 rounded-lg px-2 py-1">
                      💬 {r.commander_note}
                    </div>
                  )}
                </div>
      ...
      {view === "swap" && (
        <div className="space-y-4">
          <form
            onSubmit={submitSwap}
            className="space-y-4 bg-white rounded-2xl p-4 shadow-sm"
          >
          ...
          {mySwaps.length > 0 && (
            <div className="space-y-2">
              ...
              {mySwaps.map((sw) => (
                <div
                  key={sw.id}
                  className={`rounded-xl border p-3 ${STATUS_COLOR[sw.status] ?? ""}`}
                >
                  ...
                  <div className="text-xs text-gray-500 mt-0.5">
                    ...
                    {diffDays(sw.start_date, sw.end_date)} ימים)
                  </div>
                </div>
              ))}
            </div>
...
function RoundTab({
  ...
  const [chosen, setChosen] = useState<"א" | "ב">("א");
  const [loading, setLoading] = useState(false);

  const submit = async () => {
    setLoading(true);
    try {
      await api.createRound({ soldier_name: soldier.name, round: chosen });
      toast.success(`✅ בקשת סבב ${chosen}' נשלחה!`);
      await onRefresh();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "שגיאה");
    } finally {
  ...
  return (
    <div className="p-4 space-y-4">
      <h2 className="text-lg font-bold text-slate-800">בחירת סבב</h2>

      {myRound && (
        <div className="bg-green-50 border border-green-200 rounded-2xl p-4">
          <div className="text-green-800 font-semibold">
            ✅ סבב מאושר: סבב {myRound.round}&apos;
          </div>
          <div className="text-green-600 text-sm mt-1">
            ניתן לשנות — הבחירה החדשה תשלח לאישור מחדש
          </div>
        </div>
      )}
      ...
      {pendingRound && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4">
          <div className="text-yellow-800 font-semibold">
            ⏳ סבב {pendingRound.round}&apos; ממתין לאישור מפקד
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm space-y-4">
        <p className="text-sm text-gray-600">
          בחר את הסבב שלך. הבחירה תשלח למפקד לאישור.
        </p>

        <div className="bg-slate-50 rounded-xl p-3 text-xs text-slate-600 space-y-1">
          <div className="font-semibold">מבנה המחזור (15 ימים):</div>
          <div>יום 0: יום משותף — כולם בבסיס 🛡️</div>
          <div>ימים 1–6: סבב א&apos; בבית 🏠 | סבב ב&apos; בבסיס 🛡️</div>
          <div>ימים 7–14: סבב א&apos; בבסיס 🛡️ | סבב ב&apos; בבית 🏠</div>
        </div>
...
function CalendarTab({
    ...
    return (
      <div className="p-6 text-center text-gray-400 mt-10">
        ...
        <p className="font-medium">
          {swaps.some(
            (s) =>
              s.soldier_name === soldier.name && s.status === "Pending",
          )
            ? "הסבב שלך ממתין לאישור"
            : "לא נבחר סבב עדיין"}
        </p>
        <p className="text-sm mt-1">
          עבור ללשונית &apos;סבב&apos; כדי לבחור
        </p>
      </div>
    );
  }
  ...
  const approvedSwaps = swaps.filter(
    (s) =>
      s.status === "Approved" &&
      (s.requester === soldier.name || s.partner === soldier.name),
  );
  ...
  const classify = (dateStr: string): "leave" | "swap" | "home" | "base" => {
    ...
    return dayType(d, myRound.round);
  };
              ...
              {weeks.map((week, wi) => (
                <div key={wi} className="grid grid-cols-7 gap-0.5">
                  {week.map((day, di) => {
                    ...
                    const inRange =
                      dateStr >= "2026-04-26" && dateStr <= "2026-06-26";
                    if (!inRange) {
                      return (
                        <div
                          key={di}
                          className="aspect-square rounded-lg flex items-center justify-center text-xs text-gray-300 bg-gray-50"
                        >
                          {day}
                        </div>
                      );
                    ...
                    return (
                      <div
                        key={di}
                        className={`aspect-square rounded-lg flex flex-col items-center justify-center text-xs font-medium ${colors}`}
                      >
